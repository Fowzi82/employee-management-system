...
# for  saving inordertraversal of b tree in  a file
def dump(btree):
    with open("myApp/main.txt", 'w') as f:
        f.write(str(btree))

# for  b tree in  a file
# main purpose is to use that tree for search as inorder traversal takes O(n) time making search useless
# so i saved b tree in another file for simplicity       
def dumptree(btree):
...
def all_emp(request):
    context = {}
    # getting data from main.txt file
    emps = load()
    context = {"emps": emps}
...
def change_emp(request):
    if request.method == "POST":
        ...
        # get data
        d = loadtree()
        # using tree for O(logn)
        # as using traversal will lead to O(n)
